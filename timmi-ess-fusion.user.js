...
(async () => {
    ...
    async function sessionHandling() {
        ...
        if (window.location.href.indexOf('ZCA_TIMESHEET') !== -1 || window.location.href.indexOf('/sap/') !== -1) {
            ...
            if (isSessionTimedOut()) {
                ...
                var legacyRedirectLoginBox = $('<a href="#">Login (Legacy UI)</<a>');
                legacyRedirectLoginBox
                    .css('text-decoration','none')
                    .css('color','white')
                    .css('margin','10px')
                    .css('padding','10px 40px')
                    ...
                    .css('display','inline-block')
                    .css('border-radius','10px');
                loginBox.append(legacyRedirectLoginBox);

                legacyRedirectLoginBox.click(async function(){
                    GM.setValue('ess_sessionUrl','');
                    GM.setValue('legacyEss_sessionUrl','');
    ...
    async function appendEss() {
        ...
        var oldNewEssBtn = $('<button id="oldNewEssBtn">Login</button>');
        $('body').append(oldNewEssBtn);
        setButtonStyle(oldNewEssBtn);
        oldNewEssBtn
            .css('position','fixed')
            .css('bottom','15px')
            .css('right','290px')
        ...
        oldNewEssBtn.click(async function() {
            await GM.setValue('ess_sessionUrl', _essStartUrl);
            essIframe.attr('src', _essStartUrl);
        });
        ...
        // this is necessary if the last session cannot be found
        setTimeout(function() {
            if (areThirdPartyCookiesBlocked()) {
                // do not redirect to login if user has to allow cookies first
                return;
            }
