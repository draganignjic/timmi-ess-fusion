...
// @downloadURL  https://raw.githubusercontent.com/draganignjic/timmi-ess-fusion/master/protime-ess-fusion.user.js
// @updateURL    https://raw.githubusercontent.com/draganignjic/timmi-ess-fusion/master/protime-ess-fusion.user.js
// ==/UserScript==

(async () => {

    let _updateUrl = "https://raw.githubusercontent.com/draganignjic/timmi-ess-fusion/master/protime-ess-fusion.user.js";
    ...
    if (window.location.href.indexOf('.myProtime.eu') !== -1 && window.location.href.indexOf('/login') == -1){
        await appendEss();
        collectTimmiHours();
        fixTimmiLayout();
    }
    ...
    function fixTimmiLayout(){
        $('.viewport-panelWrapper').css('left',0);
    }
    ...
    function collectTimmiHours(){
        $('header.guBIn').each(function() {
            var date = $(this).closest('td').attr('id').split(':');
            date = date[date.length - 1];
            if (date.length != 10){
                return;
            }

            var hasWorkhours = $(this).parent().find('[data-color="lightBlue"]').length != 0;
            var hasAbsence = $(this).parent().find('[data-color="green"]').length != 0;
